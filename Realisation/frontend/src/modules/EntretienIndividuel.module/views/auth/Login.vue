            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    ...
                    <div>
                        ...
                        <div v-if="errorMessage" class="mb-4 p-3 bg-red-100 dark:bg-red-800 border border-red-300 dark:border-red-600 text-red-700 dark:text-red-200 rounded">
                            {{ errorMessage }}
                        </div>
...
import AuthService from '@/modules/EntretienIndividuel.module/service/Auth.service.js';
import { useAuthStore } from '@/modules/EntretienIndividuel.module/stores/auth.js';
...
// Updated handleLogin function in your component
const handleLogin = async () => {
  console.log('handleLogin triggered', email.value, password.value);
  ...
  try {
    ...
    localStorage.setItem('role', user.apprenant ? 'apprenant' : user.responsable ? 'responsable' : 'unknown');

    // Redirect based on role
    if (user.apprenant) {
      router.push('/entretien-individuel/student/dashboard');
